...
export default function Home() {
  ...
  async function handleSubmit(event) {
    ...
    setShortUrl(`https://short.ly/${uuidv4().slice(0, 6)}`);
  ...
  return (
    <div>
      <form onSubmit={handleSubmit}>
        <label>
          Long URL:
          <input placeholder='Enter your long url' type="text" value={longUrl} onChange={event => setLongUrl(event.target.value)} />
        </label>
        <button type="submit">Shorten</button>
      </form>
      {/* {shortUrl && <p>Short URL: <a href={shortUrl}>{shortUrl}</a></p>}
       */}
       <p id='shortenedURL'>Shortened url: <span id='short'>{shortUrl}</span></p>
    </div>
  );
}
